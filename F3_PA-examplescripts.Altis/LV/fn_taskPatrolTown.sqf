...
private ["_geararray","_grpId","_customInit","_cPos","_skls","_skills","_dir","_range","_unitType","_unit","_radius","_still","_centerPos","_menAmount","_milGroup","_menArray","_redMenArray","_pos","_allUnitsArray","_menRatio","_validPos","_whichOne","_thisArray","_smokesAndChems","_bposfinal","_bpos","_tmp","_tmpwip"];
...
_geararray = ["gren","r","rat","ar","gren","aar","matg","mmgg","sn","r","m"];
_formation = "DIAMOND"; if (random(1)>0.4) then {_formation = "FILE";};
...
_bposfinal = _tmp call BIS_fnc_selectRandom;
...
for "_i" from 1 to _menAmount do{
	...
	_pos = _bposfinal select(floor (random (count _bposfinal)));
...

{
	if (_x == leader (group _x)) then{
		...
		//} else {
		[_x, _cPos, _radius, 7, "MOVE", "SAFE", "WHITE", "NORMAL", _formation, "group _x spawn CBA_fnc_searchNearby", [15,20,40]] call CBA_fnc_taskPatrol; //STAG COLUMN
		//};	
...
[_milGroup] spawn {
	...
	sleep 16;
	//secure leader
	if ((leader _grp) distance (_poslist select _i) < 7) then {
		_notsafe = true;
		while {_notsafe} do {
			_notsafe = false;
	...
	//secure squadmembers
	{
		if (_X distance (_poslist select _i) < 7) then {
